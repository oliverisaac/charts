  ...
  config.xml: |-
    ...
    <hudson>
      ...
      <securityRealm class="org.jenkinsci.plugins.oic.OicSecurityRealm" plugin="oic-auth@1.4">
        <clientId>{{ .Values.sso.clientId }}</clientId>
        <clientSecret>{{ .Values.sso.clientSecret }}</clientSecret>
        <tokenServerUrl>{{ .Values.sso.tokenServerUrl }}</tokenServerUrl>
        <authorizationServerUrl>{{ .Values.sso.authorizationServerUrl }}</authorizationServerUrl>
        <userInfoServerUrl>{{ .Values.sso.userInfoServerUrl }}</userInfoServerUrl>
        <userNameField>{{ .Values.sso.userNameField }}</userNameField>
        <fullNameFieldName>{{ .Values.sso.fullNameFieldName }}</fullNameFieldName>
        <emailFieldName>{{ .Values.sso.emailFieldName }}</emailFieldName>
        <groupsFieldName>{{ .Values.sso.groupsFieldName }}</groupsFieldName>
        <scopes>{{ .Values.sso.scopes }}</scopes>
        <disableSslVerification>{{ .Values.sso.disableSslVerification }}</disableSslVerification>
        <logoutFromOpenidProvider>{{ .Values.sso.logoutFromOpenidProvider }}</logoutFromOpenidProvider>
        <endSessionUrl>{{ .Values.sso.endSessionUrl }}</endSessionUrl>
        <postLogoutRedirectUrl>{{ .Values.sso.postLogoutRedirectUrl }}</postLogoutRedirectUrl>
        <escapeHatchEnabled>{{ .Values.sso.escapeHatchEnabled }}</escapeHatchEnabled>
        <escapeHatchSecret>{{ .Values.sso.escapeHatchSecret }}</escapeHatchSecret>
      </securityRealm>
      {{- else }}
